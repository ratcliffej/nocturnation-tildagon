...
# Spec v3: 2-byte "NN" magic prefix + protocol_version 0x03. v0x02 -> v0x03
# added a 4-byte little-endian send_tick to LIGHT_PULSE, LIGHT_WASH,
# LIGHT_WASH_PULSE. Test vector's send_tick = 0x12345678 for exercise.
LIGHT_PULSE_VECTOR = bytes([
    ...
    0x0D,  # payload_len = 13 (v0x03; was 9 in v0x02)
    ...
    0x78, 0x56, 0x34, 0x12,   # send_tick LE = 0x12345678 (v0x03)
])

# Manual annex C.2: HEARTBEAT from source_id 1, sequence 43.
# Spec v3 §3.3.1 payload unchanged from v2: tick (u32 LE) + days_since_2026
# (u16 LE) + centiseconds_today (u24 LE). Picked tick = 0x12345678 /
# days = 0x0123 / centiseconds = 0xABCDEF to exercise each field's byte width.
HEARTBEAT_VECTOR = bytes([
    ...
    0x09,  # payload_len (HEARTBEAT unchanged in v0x03)
...
class TestHeaderParsing:
    def test_light_pulse_header_fields(self):
        ...
        assert f.protocol_version == 0x03   # v0x03
...
class TestLightCommandPayload:
    def test_light_pulse_unpacks_all_fields(self):
        ...
        assert f.send_tick == 0x12345678   # v0x03
...
class TestFrameRejection:
    ...
    def test_payload_len_mismatch_rejected(self):
        bad = bytearray(LIGHT_PULSE_VECTOR)
        bad[7] = 0x08  # payload_len offset; claim 8-byte payload, actual is 13 (v0x03)
        with pytest.raises(FrameError):
            parse_frame(bytes(bad))

    def test_wrong_payload_len_for_known_type_rejected(self):
        # HEARTBEAT MUST be 9-byte payload per spec §3.3.1.
